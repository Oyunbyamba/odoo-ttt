odoo.define("attendance_table.RenderTable", function(require) {
  ...
  var EmployeeFormRenderer = FormRenderer.extend({
    ...
    _renderTable: function(ev, data, count_table) {
        ...
        child_employees.forEach((child_emp, i) => {
          ...
          for (var header of headers) {
            if (header[0] == "field_name") {
              ...
              var total_worked_hours = convertNumToTime(
                child_emp["total_worked_hours"]
              );
              var total_schedule_hours = convertNumToTime(
                child_emp["total_schedule_hours"]
              );
              ...
              var check_in_dt =
                day_data["check_in"].length > 0
                  ? day_data["check_in"][0]["check_in"]
                  : "";
              var check_out_dt =
                day_data["check_out"].length > 0
                  ? day_data["check_out"][0]["check_out"]
                  : "";
              ...
              var $txt_check_in = $("<div>").css({
                padding: "0.1rem 0.3rem 0.2rem"
              });
              var $txt_check_out = $("<div>").css({
                padding: "0.1rem 0.3rem 0.2rem"
              });
              if (header[1] == 0) {
                day_data["check_in"].length > 0 && check_in_hours == "-"
                  ? $txt_check_in.css({ "background-color": "#ffb3b4" })
                  : "";
                day_data["check_out"].length > 0 && check_out_hours == "-"
                  ? $txt_check_out.css({ "background-color": "#ffb3b4" })
                  : "";
              }
      ...
      var $att_div = this.$el.find(".all_attendances_table");
      var $div_el =
        count_table == 1
  ...
  viewRegistry.add("all_attendances_form", EmployeeFormView);
